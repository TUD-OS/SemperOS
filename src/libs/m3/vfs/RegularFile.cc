...
static cycles_t _rdtsc() {
    uint32_t u, l;
    asm volatile ("rdtsc" : "=a" (l), "=d" (u) : : "memory");
    return (cycles_t)u << 32 | l;
}
bool RegularFile::doMemAccess = true;
...
ssize_t RegularFile::do_read(void *buffer, size_t count, Position &pos) const {
    ...
    while(count > 0) {
        ...
        if(doMemAccess)
            ...
            cycles_t start = _rdtsc();
            // 180 cycles latency + 2 cycles per byte transfer time for transfers
            cycles_t duration = 180 + amount / 8 * 2;
            while(_rdtsc() < start + duration)
                ;
        }
        buf += amount;
        count -= amount;
