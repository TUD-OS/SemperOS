...
PipeHandler::session_type *PipeHandler::handle_open(GateIStream &args) {
...
void PipeHandler::handle_obtain(session_type *, RecvBuf *, GateIStream &args, uint) {
...
void PipeHandler::handle_delegate(session_type *, GateIStream &args, uint) {
    // receive capability to client's send gate
    capsel_t msgcap = VPE::self().alloc_cap();
    _pipe = new LocalDirectPipe(msgcap, args.gate().session<Session>());
