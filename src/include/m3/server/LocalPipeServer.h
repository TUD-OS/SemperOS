...
template<class HDL>
class LocalPipeServer : public ObjCap {
    using handler_func = void (LocalPipeServer::*)(GateIStream &is);

public:
    static constexpr size_t DEF_BUFSIZE     = 8192;
    static constexpr size_t DEF_MSGSIZE     = 256;
    static const size_t MAX_RECVBUFS        = 1;
    static const int MAX_KRNL_MSGS          = 1;

    explicit LocalPipeServer(const String &name, HDL *handler, int buford = nextlog2<DEF_BUFSIZE>::val,
        ...
        : ObjCap(SERVICE, VPE::self().alloc_cap()), _handler(handler), _ctrl_handler(),
          ...
          _epid{VPE::self().alloc_ep()},
            _rcvbuf{RecvBuf::create(_epid[0], buford, msgord, 0)},
            _ctrl_rgate{RecvGate::create(&_rcvbuf[0])},
            _ctrl_sgate{SendGate::create(DEF_MSGSIZE, &_ctrl_rgate[0])} {
        LLOG(SERV, "create(" << name << ")");
        // Hack for LocalDirectPipe
        ...
        for(uint i = 0; i < _recv_bufs; i++)
            _ctrl_rgate[i].subscribe(std::bind(&LocalPipeServer::handle_message, this, _1, _2));

        _ctrl_handler[KIF::Service::OPEN] = &LocalPipeServer::handle_open;
        _ctrl_handler[KIF::Service::OBTAIN] = &LocalPipeServer::handle_obtain;
        _ctrl_handler[KIF::Service::DELEGATE] = &LocalPipeServer::handle_delegate;
        _ctrl_handler[KIF::Service::CLOSE] = &LocalPipeServer::handle_close;
        _ctrl_handler[KIF::Service::SHUTDOWN] = &LocalPipeServer::handle_shutdown;
    }
    ~LocalPipeServer() {
        LLOG(SERV, "destroy()");
    ...
    void shutdown() {
        LLOG(SERV, "shutting down server");
