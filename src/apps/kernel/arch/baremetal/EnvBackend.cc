...
public:
    ...
    virtual bool extend_heap(size_t size) {
        if(!Platform::pe_by_core(Platform::kernel_pe()).has_virtmem()){
            m3::Serial::get() << "PE " << Platform::kernel_pe() << " does not have virt mem\n";
            return false;
        }
        ...
        VPEDesc vpe(Platform::kernel_pe(), Platform::kernelId());
