...
MemoryModule MainMemory::detach(size_t size) {
    for(int i = _count - 1; i >= 0; --i) {
        if(_mods[i]->available()) {
            uintptr_t addr = _mods[i]->map().detach(size);
            if(addr == static_cast<uintptr_t>(-1))
                continue;
            return MemoryModule(true, _mods[i]->pe(), addr, size);
        }
    }
    return MemoryModule(false, 0, 0, 0);
}
