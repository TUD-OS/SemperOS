...
void OpDescr::extractValues(const string &str, size_t numArgs,
    ...
    for (unsigned int i = 0; i < numArgs; i++) {
        ...
        cout << "extracing from substr: " << str.substr(pos, string::npos) << "\n";
        len = argLen(str, pos, newPos);
        if (len == string::npos)
            break;
cout << "extract params pos: " << pos << " length: " << len << "\n";
...
void OpDescr::extractSubValues(const string &str, size_t numArgs,
                            ArgsVector &args) const {
    ...
    // extract arguments, in subvalues that are prepended with the struct member's name
    pos = str.find('=') + 1;
    for (unsigned int i = 0; i < numArgs; i++) {
        cout << "extracing from substr: " << str.substr(pos, string::npos) << "\n";
        len = subArgLen(str, pos, newPos);
        if (len == string::npos)
            break;
        cout << "extract params pos: " << pos << " length: " << len << "\n";
        args.push_back(str.substr(pos, len));
        pos = str.find('=', newPos) + 1;
    }
}
...
size_t OpDescr::argLen(string const &str, size_t pos, size_t &newPos) const {
    ...
    bool   isSquareBracket = (str[pos] == '[');
    bool   isCurlyBrace = (str[pos + 1] == '{');

    if (isSquareBracket && !isCurlyBrace)
        pos++;

    // if argument sub string is enclosed in square brackets, we look for the a
    // closing bracket; both must not be preceded by a backslash
    if (isCurlyBrace || isSquareBracket) {
        ...
        // skip to the character after the closing bracket or brace
        i++;
        newPos = i + 2;
    ...
    if (i == string::npos) {
            i = str.find(')', pos);
            size_t retValStart = str.find_last_of('=');
            // if closing bracket is after last '=' it belongs to the error string,
            // thus it's not part of the arguments
            if (i > retValStart)
                i = string::npos;
    }
...
size_t OpDescr::subArgLen(string const &str, size_t pos, size_t &newPos) const {
...
class OpenAtOpDescr: public OpDescr {
public:
    OpenAtOpDescr(const std::string &str);
};
...
class NewFStatAtOpDescr: public OpDescr {
public:
    NewFStatAtOpDescr(const std::string &str);
};
...
class SelectOpDescr: public OpDescr {
public:
    SelectOpDescr(const std::string &str);
};
...
class WritevFromOpDescr: public OpDescr {
public:
    WritevFromOpDescr(const std::string &str);
};
...
OpenAtOpDescr::OpenAtOpDescr(const string &in) {

    string retVal;
    ArgsVector args(4);

    extractValues(in, 4, args, retVal);
    buildCodeLine("OPENAT_OP", "openat",
                  retVal + ", " + args[0] + ", " + args[1] + ", " + args[2] +
                  ", " + ((args.size() > 3) ? args[3] : "0"));
}
...
NewFStatAtOpDescr::NewFStatAtOpDescr(const string &in) {

    string retVal;
    ArgsVector args(1);

    extractValues(in, 2, args, retVal);
    buildCodeLine("NEWFSTATAT_OP", "newfstatat", retVal + ", " + args[0] + ", " +
            args[1]);
}
...
SelectOpDescr::SelectOpDescr(const string &in) {

    string retVal;
    ArgsVector args(0);

    extractValues(in, 0, args, retVal);
    buildCodeLine("SELECT_OP", "select",
                    retVal);
    // TODO
//                  retVal + ", " + args[0] + ", " + args[1] + ", " + args[2]
//                  + ", " + args[3] + ", " + args[4]);
...
RecvFromOpDescr::RecvFromOpDescr(const string &in) {
    ...
    extractValues(in, 6, args, retVal);
    buildCodeLine("RECVFROM_OP", "recvfrom",
                  retVal + ", " + args[0] + ", " + args[1] + ", " + args[2]
...
WritevFromOpDescr::WritevFromOpDescr(const string &in) {

    string retVal;
    ArgsVector args(3);
    string iovret;
    ArgsVector iovargs(2);
    cout << "writev Op\n";
    extractValues(in, 3, args, retVal);
    cout << "writev op subvalues: " << args[1] << "\n";
    extractSubValues(args[1], 2, iovargs);
    cout << "writev done\n";
    buildCodeLine("WRITEV_OP", "writev",
                  retVal + ", " + args[0] + ", .iovec = { " + iovargs[0] + " , " +
                  iovargs[1] + " }, " + args[2]);
}
...
OpDescr *OpDescrFactory::create(const string &line) {
    ...
    if (OpDescrFactory::isStringHead(line, "openat("))
        return new OpenAtOpDescr(line);
    ...
    if (OpDescrFactory::isStringHead(line, "newfstatat("))
        return new NewFStatAtOpDescr(line);
