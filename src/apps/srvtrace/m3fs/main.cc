...
int main(int argc, char **argv) {
    ...
    m3::String pipesrvname("pipe");
    if(argc > 2)
        pipesrvname = IStringStream::read_from<m3::String>(argv[2]);
    ...
    pipesrv = new LocalPipeServer<PipeHandler>(pipesrvname, &pipeHndlr);
    env()->workloop()->run();
    globpipe = pipeHndlr.pipe();
    // don't destroy the service, so far this leads to revocation issues
//    delete pipesrv;
    if(globpipe == nullptr) {
        exitmsg("Broken pipe");
        return 1;
