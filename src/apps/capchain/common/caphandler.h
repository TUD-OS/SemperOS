...
public:
    CapHandler(bool isRoot, int numcaps, CapServerSession *sess = nullptr) : reqhandler_t(),
    ...
    void signal(GateIStream &) {
        CAPLOGV("signal()");
        if(!_sess) {
            _sess = new CapServerSession("capchain-root");
            if(Errors::occurred()) {
                CAPLOG("Unable to create sess at capchain-root server. Error= " << Errors::last <<
        ...
        if(_isRoot && _obtained == _numcaps - 1) {
            revcap();
