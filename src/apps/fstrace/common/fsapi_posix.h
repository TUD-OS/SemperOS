...
public:
    ...
    virtual void openat(const openat_args_t *args, int lineNo) override {
        // only supporting absolute paths
        if(strncmp(args->name, "/", 1) != 0)
            THROW(IllegalArgumentException);
        int fd = ::open(args->name, args->flags, 0666 /*args->mode*/);
        if (fd < 0 && args->fd >= 0)
            THROW1(ReturnValueException, -errno, args->fd, lineNo);
        else if (fd >= 0)
            fdMap[args->fd] = fd;
    ...
    virtual void newfstatat(const newfstatat_args_t *, int) override {
        THROW(LogableException);
    }
    ...
    virtual void select(const select_args_t *, int ) override {
        // TODO not implemented
    }

    virtual void recvfrom(const recvfrom_args_t *, int ) {
        // TODO not implemented
    }

    virtual void modsendfile(Buffer &, const sendfile_args_t *, int ) {
        // TODO not implemented
    }

    virtual void writev(const writev_args_t *, int ) {
        // TODO not implemented
    }
