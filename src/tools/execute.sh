...
build_params_gem5() {
    ...
    params=`mktemp`
    echo -n "--outdir=$M3_GEM5_OUT --debug-file=gem5.log" >> $params
    if [ "$M3_GEM5_DBG" != "None" ]; then
        echo -n " --debug-flags=$M3_GEM5_DBG" >> $params
    fi
    # no stats
    echo -n " --stats-file=/dev/null" >> $params
    # no config
    if [ "$M3_GEM5_NOCFGDUMP" == "1" ]; then
        echo -n " --dump-config=/dev/null --json-config=/dev/null" >> $params
    fi
    if [ "$M3_DOT_CONFIG" == "" ]; then
        echo -n " --dot-config=None" >> $params
    else
        echo -n " --dot-config=$M3_DOT_CONFIG" >> $params
    fi
    if [ "$M3_DBG_START" != "" ]; then
        echo -n " --debug-start=$M3_DBG_START" >> $params
    fi
    echo -n " $M3_GEM5_CFG --cpu-type $M3_GEM5_CPU --cmd \"$cmd\"" >> $params
    echo -n " --cpu-clock=2GHz --sys-clock=1GHz" >> $params
    if [ "$M3_PAUSE_PE" != "" ]; then
        echo -n " --pausepe=$M3_PAUSE_PE" >> $params
    else
        echo -n " --remote-gdb-port=0" >> $params
    fi
