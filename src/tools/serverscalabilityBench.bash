...
evaluation=0
# server instances per load generator
serverPerLoadgen=9
...
clients=(1 4 8 16 32 64 96 128 160 192 224 256 320 384 448)
...
for k in `seq $((${#clients[@]} - 1)) -1 0`; do
    ...
    for j in `seq 0 $((${#fs[@]} - 1))`; do
        ...
        for i in `seq 0 $((${#krnls[@]} - 1))`; do
            ...
            sh ./b run boot/servertrace.cfg &>$logdir/log.txt &
            execID=$(($!))
            runningSims+=($execID)
            simPIDs+=($execID)
            nextSimID+=1
